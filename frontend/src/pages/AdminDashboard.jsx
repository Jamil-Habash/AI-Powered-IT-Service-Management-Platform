...
export default function AdminDashboard() {
    ...
    if (!selectedUser || !window.confirm(`Activate ${selectedUser.name || "this account"}?`)) return;
    setUserModalError("");
    setSavingUser(true);
    try {
        ...
        setUserModalError(err.response?.data?.error || "Unable to activate this account.");
    } finally {
        setSavingUser(false);
    }
    };
    ...
    <Shell>
        ...
        <section className="panel admin-directory">
            ...
            <div className="bulk-toolbar" aria-live="polite">
                ...
                <button className="primary-button" onClick={bulkChangeRole} disabled={bulkSaving || !bulkRole}>
            ...
            <div className="admin-table-scroll">
                ...
                <tbody>
                    ...
                    const itemTickets = tickets.filter(
                        (ticket) => ticket.createdById === item.id || ticket.assignedAgentId === item.id
                    );
                    ...
                    return (
                        ...
                        <td>
                            <span className={`account-status ${isActive ? "active" : "inactive"}`}>
                            <i />{isActive ? "Active" : "Inactive"}
                            </span>
                            <span className={`account-status ${isVerified ? "active" : "inactive"}`}>
                            <i />{isVerified ? "Verified" : "Not Verified"}
                            </span>
                        </td>
                        <td>{item.role?.replace("_", " ") || "Ticket assignment"}</td>
                        <td>
                        ...
                        <td>
                            ...
                            <button
                                ...
                                title="View details"
                                onClick={() => item.id && navigate(`/settings?user=${item.id}`)}
                            >
                                <Icon>more_vert</Icon>
                            </button>
                            )}
            ...
            <span>
                Showing {filtered.length ? (page - 1) * pageSize + 1 : 0} to {Math.min(page * pageSize, filtered.length)} of {filtered.length} records
            </span>
            <div>
        ...
        {selectedUser && (
            ...
            >
                ...
                <div>
                    ...
                    <h2>Edit {selectedUser.role === "IT_AGENT" ? "IT Agent" : "Employee"}</h2>
                </div>
                <button type="button" className="icon-button" aria-label="Close user editor" onClick={() => setSelectedUser(null)}>
                    <Icon>close</Icon>
                </button>
                </div>
                <p className="modal-lead">
                Update directory details for {selectedUser.name || "this account"}. You can also change this account's access role.
                </p>
                <label>
                Full Name
                <input value={userForm.name} onChange={updateUserField("name")} required />
                </label>
                <label>
                Work Email
                <input type="email" value={userForm.email} onChange={updateUserField("email")} required />
                </label>
                <label>
                Access Role
                <select value={userForm.role} onChange={updateUserField("role")} required>
                    <option value="EMPLOYEE">Employee</option>
                    <option value="IT_AGENT">IT Agent</option>
                    <option value="ADMIN">Administrator</option>
                </select>
                ...
                <button type="button" className="secondary-button" disabled={savingUser} onClick={() => setSelectedUser(null)}>
                    Cancel
                </button>
