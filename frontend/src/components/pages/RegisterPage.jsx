...
function FieldIcon({ path }) { return <span className="field-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d={path} strokeLinecap="round" strokeLinejoin="round" /></svg></span> }
...
export default function RegisterPage() {
  ...
  const submit = (event) => { event.preventDefault(); const confirm = event.currentTarget.elements.confirmPassword; confirm.setCustomValidity(form.password === form.confirmPassword ? '' : 'Passwords do not match'); if (!event.currentTarget.checkValidity()) { event.currentTarget.reportValidity(); return } window.alert('Account created successfully!') }
    ...
    <main className="registration-card">
        <header className="card-header">
            ...
            <form className="registration-form" onSubmit={submit}>
                ...
                {field('email', 'Email Address', mailPath, { type: 'email', placeholder: 'alex@company.com', value: form.email, onChange: update('email') })}
                <label htmlFor="password">Password</label>
                <div className="input-wrap">
                    <FieldIcon path={lockPath} />
                    <input id="password" name="password" type={visible.password ? 'text' : 'password'} placeholder="Create a strong password" minLength="8" value={form.password} onChange={update('password')} required />
                    <button className="visibility-button" type="button" onClick={() => setVisible((current) => ({ ...current, password: !current.password }))} aria-label="Toggle password visibility">
                ...
                <div className="input-wrap">
                    <FieldIcon path={lockPath} />
                    <input id="confirmPassword" name="confirmPassword" type={visible.confirmPassword ? 'text' : 'password'} placeholder="Re-enter your password" value={form.confirmPassword} onChange={update('confirmPassword')} required />
                    <button className="visibility-button" type="button" onClick={() => setVisible((current) => ({ ...current, confirmPassword: !current.confirmPassword }))} aria-label="Toggle confirm password visibility">
