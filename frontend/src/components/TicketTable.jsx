...
export default function TicketTable({ rows, onSelect }) {
  return (
    <div className="table-scroll">
      <table>
        <thead>
          <tr><th>Ticket Details</th><th>Category</th><th>Status</th><th>Priority</th><th>Created</th><th /></tr>
        </thead>
        <tbody>
          {rows.map(([id, title, category, status, priority]) => (
            <tr key={id} onClick={onSelect}>
              <td>
                ...
                <strong>{title}</strong>
                <small>Connected to service infrastructure; support team is reviewing the request.</small>
              </td>
              ...
              <td><small>Today, 10:15 AM</small><small>2 hours ago</small></td>
              <td><Icon>chevron_right</Icon></td>
            </tr>
          ))}
        </tbody>
      </table>
